    ...
    <div className="min-h-screen bg-gray-100 dark:bg-gray-900">
      ...
      >
        ...
        <div className="px-4 py-4 border-b border-primary-600 dark:border-gray-700">
          ...
          {/* Attendance Summary */}
          <div className="mt-4 bg-primary-800 dark:bg-gray-700 rounded-lg p-3">
            <p className="text-xs text-primary-200 dark:text-gray-300">Overall Attendance</p>
            <div className="flex items-center mt-1">
              <div className="flex-1 bg-gray-200 rounded-full h-2">
                <div
                  className="bg-green-500 h-2 rounded-full"
                  style={{ width: `${user?.attendance?.overall || 0}%` }}
                ></div>
              </div>
              <span className="ml-2 text-sm text-white">{user?.attendance?.overall || 0}%</span>
            </div>
          </div>
        </div>

        <nav className="mt-5 px-2">
          {NAV_ITEMS.STUDENT.map((item) => (
            <a
              key={item.path}
              href={item.path}
              onClick={(e) => {
          ...
          <div className="px-4 mt-6">
            <div className={`rounded-lg p-3 ${
              ...
              <p className="text-sm font-medium text-white">
                {user.subscription.isActive ? 'Premium Member' : 'Free Plan'}
              </p>
              {!user.subscription.isActive && (
                <button
                  onClick={() => navigate('/student/subscription')}
                  className="mt-2 w-full text-xs bg-white text-yellow-700 py-1 px-2 rounded"
                >
                  Upgrade to Premium
                </button>
              )}
          ...
          <div className="flex-1 px-4 flex justify-between">
            <div className="flex-1 flex items-center">
              <h1 className="text-2xl font-semibold text-gray-900 dark:text-white">
                {NAV_ITEMS.STUDENT.find((item) => item.path === location.pathname)?.name || 'Dashboard'}
              </h1>
            </div>
